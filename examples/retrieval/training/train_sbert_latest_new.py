...
# print package versions and file locations
import importlib
for pkg in ("torch","transformers","sentence_transformers"):
    try:
        m = importlib.import_module(pkg)
        print(pkg, "version:", getattr(m, "__version__", None), "file:", getattr(m, "__file__", None))
    except Exception as e:
        print(pkg, "import failed:", e)
# print CUDA-visible env and devices
...
# Explicitly set the model similarity function to JS divergence so evaluator and model agree
model.similarity_fn_name = "js_divergence"
# model.similarity_fn_name = "cosine"  
print("Model similarity function:", model.similarity_fn_name)
# save_dir = os.path.join(pathlib.Path(__file__).parent.absolute(), "output", f"{model_name}-fever-lr1e-5-epochs10-temperature20_full_dev_cos_sim_full_vector") #Make sure LR and temperature (scale) are correct
save_dir = os.path.join(pathlib.Path(__file__).parent.absolute(), "output", f"{model_name}-dbpedia-entity-lr1e-5-epochs10-temperature20_full_dev_cos_sim_full_vector") #Make sure LR and temperature (scale) are correct
...
ir_evaluator = InformationRetrievalEvaluator(
    ...
    name="dbpedia-entity-dev", show_progress_bar=True
)
...
trainer = SentenceTransformerTrainer(
    ...
    # Pass the correct keyword and callable so the loss uses the model's similarity (cosine here)
    loss=losses.MultipleNegativesRankingLoss(model=model, similarity_fct=model.similarity),
